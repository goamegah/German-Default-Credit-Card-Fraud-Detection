...
class ComputeModel:
    ...
    def bias_variance_estimate(self, bootstrap_rounds=100):
        ...
        X_train, y_train = pd.DataFrame(self.dict_split["arrays"]["X_train"]), pd.DataFrame(
            self.dict_split["arrays"]["y_train"].reshape(-1))
        X_test, y_test = pd.DataFrame(self.dict_split["arrays"]["X_test"]), pd.DataFrame(
            self.dict_split["arrays"]["y_test"].reshape(-1))
        ...
        for r in range(bootstrap_rounds):
            ...
            preds_test[f'Model {r}'] = estimator.fit(np.array(X_train.iloc[boot, :]),
                                                     np.array(y_train.iloc[boot]).ravel()).predict(X_test)
